...
our %config = (
  ...
  ARFLAGS => [ "r" ],
  CC => "gcc",
  CFLAGS => [ "-Wall -O3", "-O3", "-Ofast", "-w", "-m64", "-mtune=native" ],
  CPPDEFINES => [  ],
  CPPFLAGS => [  ],
  CPPINCLUDES => [  ],
  CXX => "g++",
  CXXFLAGS => [ "-Wall -O3", "-O3", "-Ofast", "-w", "-m64", "-mtune=native" ],
  ...
  openssldir => "",
  options => "--prefix=/opt/compiled/openresty/openresty-opt/openssl/.openssl -O3 -Ofast -w -m64 -mtune=native no-asan no-buildtest-c++ no-crypto-mdebug no-crypto-mdebug-backtrace no-devcryptoeng no-dynamic-engine no-ec2m no-ec_nistp_64_gcc_128 no-egd no-external-tests no-fuzz-afl no-fuzz-libfuzzer no-heartbeats no-md2 no-msan no-rc5 no-sctp no-shared no-ssl-trace no-ssl3 no-ssl3-method no-threads no-ubsan no-unit-test no-weak-ssl-ciphers no-zlib no-zlib-dynamic",
  perl_archname => "x86_64-linux-thread-multi",
  perl_cmd => "/usr/bin/perl",
  perl_version => "5.16.3",
  perlargv => [ "linux-x86_64", "--prefix=/opt/compiled/openresty/openresty-opt/openssl/.openssl", "no-shared", "no-threads", "no-ec2m", "no-ssl2", "no-ssl3", "no-weak-ssl-ciphers", "-O3", "-Ofast", "-w", "-m64", "-mtune=native" ],
  perlenv => {
      "AR" => undef,
      "BUILDFILE" => undef,
      "CC" => undef,
  ...
  prefix => "/opt/compiled/openresty/openresty-opt/openssl/.openssl",
  processor => "",
